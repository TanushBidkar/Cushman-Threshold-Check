    ...
    def calculate_quantity_ranges(self, training_data):
        ...
        for sqfeet_range, range_data in self.sqfeet_ranges.items():
            ...
            for particular, quantities in range_data['particulars_data'].items():
                ...
                # We need to get the unit from the first record of this particular
                ...
                overall_stats.update({
                    ...
                    'unit': unit_for_particular  # NEW: Store unit in stats
                })
...
def main():
    ...
    model_path = os.path.join(output_folder, "trained_model_sqfeet_zero_aware_ICICI.pkl")
    validator.save_model(model_path)
   
    validator.display_training_summary()
   
    # Generate enhanced detailed report
    report_path = os.path.join(output_folder, "training_report_sqfeet_zero_aware.txt")
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("CWI Validator Enhanced Training Report (Zero-Aware Statistics)\n")
        f.write("="*80 + "\n\n")
