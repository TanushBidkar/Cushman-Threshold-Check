...
class CWIValidator:
    def __init__(self):
        ...
        self.sqfeet_ranges = {
            '500-1000': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '1001-1500': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '1501-2000': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '2001-2500': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '2501-3000': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '3001-3500': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '3501-4000': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '4001-4500': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '4501-5000': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)},
            '5000+': {'quantity_ranges': {}, 'particulars_data': defaultdict(list), 'particulars_file_data': defaultdict(list)}
        }
    ...
    def extract_sqfeet_from_filename(self, filename):
        ...
        # 1. UPDATED PATTERNS for 500 sqft intervals
        patterns = [
        ...
        # 2. UPDATED NUMERIC LOGIC for 500 sqft intervals
        numbers = re.findall(r'\d+', filename_lower)
        if numbers:
            for num_str in numbers:
                num = int(num_str)
                
                # Logic changed to 500 steps
                if 500 <= num <= 1000:
                    return '500-1000'
                elif 1001 <= num <= 1500:
                    return '1001-1500'
            ...
            try:
                ...
                for idx, row in df_renamed.iterrows():
                    ...
                    if pd.isna(unit) or str(unit).strip() == '':
                        unit = self.infer_unit_from_particular(particular)
                        print(f"  → Inferred unit '{unit}' for: {str(particular)[:50]}...")
                    ...
                    extracted_qty = self.round_quantity_if_number_based(extracted_qty, unit)
    ...
    def calculate_quantity_ranges(self, training_data):
        ...
        print("\nCalculating quantity ranges by Sq Feet categories (EXCLUDING ZEROS FROM WEIGHTED AVG)...")
        ...
        for record in training_data:
            ...
            # NEW: Store file-level data for tracking zeros
        ...
        for sqfeet_range, range_data in self.sqfeet_ranges.items():
            ...
            for particular, quantities in range_data['particulars_data'].items():
                ...
                # ✅ CORRECTED: Calculate WEIGHTED AVERAGE RANGE (EXCLUDING ZEROS) - mean ± 1*std
                if len(non_zero_qty) > 0:
                    non_zero_mean = float(np.mean(non_zero_qty))
                    non_zero_std = float(np.std(non_zero_qty))
                    ...
                    if non_zero_std > 0:
                        ...
                        # ✅ CRITICAL FIX: Cap weighted_avg_max at overall_max
                        weighted_avg_max = min(weighted_avg_max, overall_stats['overall_max'])

                        # ✅ CRITICAL FIX: Cap weighted_avg_min at overall_min (should not be less)
                        weighted_avg_min = max(weighted_avg_min, overall_stats['overall_min'])
                    else:
                ...
                # ✅ CORRECTED: Store file information for ACTUAL threshold min/max (not weighted avg)
                file_data = range_data['particulars_file_data'][particular]

                # Find files with quantities matching the ACTUAL threshold min/max
                min_files = []
                max_files = []
                ...
                # Round if number-based for comparison
                comparison_min = round(actual_min) if (unit_for_particular and self.is_number_based_unit(unit_for_particular)) else actual_min
                comparison_max = round(actual_max) if (unit_for_particular and self.is_number_based_unit(unit_for_particular)) else actual_max

                for item in file_data:
                    item_qty = item['quantity']
                    # Round item quantity if number-based
                    if unit_for_particular and self.is_number_based_unit(unit_for_particular):
                        item_qty = round(item_qty)

                    # Check if quantity matches ACTUAL min (with small tolerance for floats)
                    if abs(item_qty - comparison_min) < 0.01:
                        min_files.append(item)

                    # Check if quantity matches ACTUAL max (with small tolerance for floats)
                    if abs(item_qty - comparison_max) < 0.01:
                ...
                overall_stats.update({
                    ...
                    'min_threshold_files': min_files,  # Files with quantity = weighted_avg_min
                    'max_threshold_files': max_files   # Files with quantity = weighted_avg_max
                })
    ...
    def validate_quantity(self, particular, quantity, sqfeet_range):
        """✅ CHANGED: Validate quantity using WEIGHTED AVERAGE ranges (excluding zeros)"""
        ...
        if particular in range_data['quantity_ranges']:
            ...
            # ✅ CHANGED: Use weighted averages for validation
            ...
            # Get unit and round if number-based
            unit = stats.get('unit', None)
            if unit and self.is_number_based_unit(unit):
                validation_min = round(validation_min)
                validation_max = round(validation_max)
                threshold_min = round(threshold_min)
                threshold_max = round(threshold_max)
            ...
            if quantity == 0 and stats['zero_allowed']:
                return True, f"Zero quantity allowed (appears in {stats['zero_count']}/{stats['total_count']} training cases)", threshold_min, threshold_max, validation_min, validation_max, min_threshold_files, max_threshold_files

            # ✅ CHANGED: Validate against weighted average range
            if unit and self.is_number_based_unit(unit):
                is_valid = validation_min <= round(quantity) <= validation_max
            else:
                is_valid = validation_min <= quantity <= validation_max

            # Create message with rounded values if number-based
            if unit and self.is_number_based_unit(unit):
        ...
        if similar_particular:
            ...
            # Special handling for zero quantities
            if quantity == 0 and stats['zero_allowed']:
                return True, f"Zero quantity allowed via similar match ({similarity:.2f}) - appears in {stats['zero_count']}/{stats['total_count']} training cases", threshold_min, threshold_max, validation_min, validation_max, min_threshold_files, max_threshold_files
    ...
    def display_training_summary(self):
        ...
        print("\n" + "="*70)
        print("COMPREHENSIVE TRAINING SUMMARY (WEIGHTED AVG EXCLUDES ZEROS)")
        print("="*70)
        ...
        for sqfeet_range, range_data in self.sqfeet_ranges.items():
            ...
            if range_particulars > 0:
                ...
                weighted_ranges = [stats['weighted_avg_max'] - stats['weighted_avg_min']
                                   for stats in range_data['quantity_ranges'].values() if 'weighted_avg_max' in stats]
        ...
        print("\nModel is ready for validation with WEIGHTED AVG (EXCLUDING ZEROS)!")
        print("="*70)

def main():
    print("CWI Validator - Enhanced Training Phase (Weighted Avg Excludes Zeros)")
    print("="*60)
   
    validator = CWIValidator()
   
    training_folder = r"C:\Users\Tanush.Bidkar\Downloads\CWI_Validator_ICICI\training_files"
    ...
    output_folder = r"C:\Users\Tanush.Bidkar\Downloads\CWI_Validator_ICICI"
   
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    model_path = os.path.join(output_folder, "trained_model_sqfeet_weighted_avg_ICICI.pkl")
    validator.save_model(model_path)
   
    validator.display_training_summary()
   
    # Generate enhanced detailed report
    report_path = os.path.join(output_folder, "training_report_sqfeet_weighted_avg.txt")
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("CWI Validator Enhanced Training Report (Weighted Avg Excludes Zeros)\n")
        f.write("="*80 + "\n\n")
        ...
        for sqfeet_range, range_data in validator.sqfeet_ranges.items():
            if len(range_data['quantity_ranges']) > 0:
                ...
                for particular, stats in range_data['quantity_ranges'].items():
                    ...
                    # Write zero files information
                    if stats.get('zero_files'):
                        f.write(f"  Files with Zero Values:\n")
                        for zf in stats['zero_files'][:5]:  # Show first 5
                            f.write(f"    - {zf['file']}: {zf['original_particular']}\n")
                        if len(stats['zero_files']) > 5:
                            f.write(f"    ... and {len(stats['zero_files']) - 5} more\n")
